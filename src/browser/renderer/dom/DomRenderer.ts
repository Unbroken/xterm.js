...
export class DomRenderer extends Disposable implements IRenderer {
  ...
  private _injectCss(colors: ReadonlyColorSet): void {
    ...
    for (const [i, c] of colors.ansi.entries()) {
      styles +=
        `${this._terminalSelector} .${Constants.FG_CLASS_PREFIX}${i} { color: ${toCssColor(c.css, colorSpace)}; }` +
        `${this._terminalSelector} .${Constants.FG_CLASS_PREFIX}${i}.${RowCss.DIM_CLASS} { color: ${toCssColor(color.multiplyOpacity(c, 0.5).css, colorSpace)}; }` +
        `${this._terminalSelector} .${Constants.BG_CLASS_PREFIX}${i} { background-color: ${toCssColor(c.css, colorSpace)}; }`;
    }
    styles +=
      `${this._terminalSelector} .${Constants.FG_CLASS_PREFIX}${INVERTED_DEFAULT_COLOR} { color: ${toCssColor(color.opaque(colors.background).css, colorSpace)}; }` +
      `${this._terminalSelector} .${Constants.FG_CLASS_PREFIX}${INVERTED_DEFAULT_COLOR}.${RowCss.DIM_CLASS} { color: ${toCssColor(color.multiplyOpacity(color.opaque(colors.background), 0.5).css, colorSpace)}; }` +
      `${this._terminalSelector} .${Constants.BG_CLASS_PREFIX}${INVERTED_DEFAULT_COLOR} { background-color: ${toCssColor(colors.foreground.css, colorSpace)}; }`;
