...
export class TextureAtlas implements ITextureAtlas {
  ...
  private _drawToCache(codeOrChars: number | string, bg: number, fg: number, ext: number, restrictToCellHeight: boolean, domContainer: HTMLElement | undefined): IRasterizedGlyph {
    ...
    this._tmpCtx.textRendering = this._config.disableFontHinting ? 'geometricPrecision' : 'auto';
    ...
    if (this._config.customGlyphs !== false) {
      ...
      customGlyph = tryDrawCustomGlyph(this._tmpCtx, chars, padding, padding, this._config.deviceCellWidth, this._config.deviceCellHeight, this._config.deviceCharWidth, this._config.deviceCharHeight, this._config.fontSize, this._config.devicePixelRatio, backgroundColor.css, variantOffset, this._config.colorSpace);
    }
...
class AtlasPage {
   ...
   * page and for a page object swap at the same index.
   */
